...
```shell
➜ go get github.com/golangci/golangci-lint/cmd/root@
```
...
getdeps:
   @mkdir -p {GOPATH}/bin
   @which golangci-lint 1>/dev/null || (echo "Installing golangci-lint" && go get github.com/golangci/golangci-lint/cmd/@)
lint:
   @echo "Running @ check"
