...
前面几课时我分别介绍了 Docker 常见的联合文件系统解决方案：AUFS 和 Device Mapper。今天介绍一个性能更好的联合文件系统解决方案——OverlayFS。

OverlayFS 的发展分为两个阶段。2014 年，OverlayFS 第一个版本被合并到 Linux 内核 3.18 版本中，此时的 OverlayFS 在 Docker 中被称为 `overlay` 文件驱动。由于第一版的 `overlay` 文件系统存在很多弊端（例如运行一段时间后 Docker 会报“too many links problem”错误），Linux 内核在 4.0 版本对 `overlay` 做了很多必要的改进，此时的 OverlayFS 被称为 `overlay2`。

因此，Docker 中的 OverlayFS 文件驱动分为两种：早期的 `overlay` 不推荐在生产环境中使用，更新且更稳定的 `overlay2` 则推荐用于生产环境。下面主要围绕 `overlay2` 展开。
...
- `overlay2` 最好搭配 XFS 文件系统使用，并且必须开启 d_type。可以使用以下命令验证 d_type 是否开启：
...
另外，在生产环境中，推荐将 /var/lib/docker 目录挂载到单独的磁盘或分区，这样可以避免该目录写满影响主机的文件写入，并且把挂载信息写入到 /etc/fstab，防止机器重启后挂载信息丢失。
...
其中 UUID 是 /var/lib/docker 所在磁盘或分区的 UUID 或磁盘路径。如果操作系统无法满足上面的条件，可以考虑 AUFS 或 Device Mapper 作为 Docker 的存储驱动。

> 通常情况下，overlay2 比 AUFS 和 Device Mapper 性能更好，也更加稳定，因为它在 inode 优化上更高效。因此，生产环境推荐使用 overlay2 作为 Docker 的存储驱动。
...
1. 使用 `mkfs` 命令格式化磁盘 vdb1：

```shell
sudo mkfs.xfs -f -n ftype=1 /dev/vdb1
```

1. 将挂载信息写入到 /etc/fstab，保证机器重启挂载目录不丢失：

```shell
echo "/dev/vdb1 /var/lib/docker xfs defaults,pquota 0 0" | sudo tee -a /etc/fstab
```

1. 使用 `mount` 命令使得挂载目录生效：
...
1. 备份 /var/lib/docker 目录：

```shell
sudo cp -au /var/lib/docker /var/lib/docker.back
```

1. 在 /etc/docker 目录下创建 `daemon.json` 文件；如果该文件已经存在，则修改为以下内容：
...
其中 `storage-driver` 指定使用 overlay2，`overlay2.size` 将每个容器可写层限制为 20 GiB。该限制依赖 XFS 的 pquota 特性，可以根据生产环境的磁盘容量调整；启用它可以避免单个容器写满整个 Docker 数据目录。
...
可以看到 Storage Driver 已经变为 overlay2，并且 d_type 也是 true。至此，Docker 配置完成。下面介绍 overlay2 的工作方式。

### overlay2 工作原理

#### overlay2 是如何存储文件的？

overlay2 和 AUFS 类似，将目录组织为多个层（layer）。overlay2 的目录是镜像和容器分层的基础，把这些层统一展现到同一目录的过程称为联合挂载（union mount）。overlay2 把下层目录称为 `lowerdir`，上层目录称为 `upperdir`，联合挂载后的结果称为 `merged`。

> overlay2 文件系统最多支持 128 个层数叠加，也就是说你的 Dockerfile 最多只能叠加 128 层（Dockerfile 指令数量不等于层数），不过这在日常使用中足够了。
...
link 和 lower 文件与镜像层的功能一致，**link** 文件内容为该容器层的短 ID，**lower** 文件为该层所有父层镜像的短 ID。**diff 目录为容器的读写层，容器内修改的文件都会在 diff 中出现，merged 目录为分层文件联合挂载后的结果，也是容器内的工作目录。** 总体来说，overlay2 的文件存储方式可以概括为：`overlay2` 将镜像层和容器层都放在单独的目录，并且有唯一 ID，每一层仅存储发生变化的文件，最终使用联合挂载技术将容器层和镜像层的所有文件统一挂载到容器中，使得容器中看到完整的系统文件。

#### overlay2 如何读取、修改文件？

overlay2 的文件操作分为读取和修改。
...
> overlay2 写时复制会复制整个文件，如果文件过大，将会大大降低文件系统的性能，因此当我们有大量文件需要被修改时，overlay2 可能会出现明显的延迟。好在，写时复制操作只在第一次修改文件时触发，对日常使用没有太大影响。

- 删除文件或目录：当文件或目录被删除时，overlay2 并不会真正从镜像中删除它，因为镜像层是只读的，overlay2 会创建一个特殊的文件或目录，这种特殊的文件或目录会阻止容器的访问。

### 结语

overlay2 是 Docker 官方推荐且默认使用的存储驱动，因为 overlay2 在生产环境中不仅有着较高的性能，它的稳定性也极其突出。但是 overlay2 的使用还是有一些限制条件的，例如要求 Docker 版本必须高于 17.06.02，内核版本必须高于 4.0 等。因此，在生产环境中，如果你的环境满足使用 overlay2 的条件，请尽量使用 overlay2 作为 Docker 的联合文件系统。
