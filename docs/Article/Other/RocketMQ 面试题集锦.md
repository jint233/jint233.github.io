...
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-1.png)
...
MessageListenerOrderly 底层会将 MessageQueue 锁住，保证同一个 MessageQueue 的消息不会并发消费。
...
1. 创建一个 Topic，并将积压 Topic 的消息分发到新 Topic 的更多分区中；新 Topic 的分区数可设为原 Topic 的 10 倍（具体按业务场景决定）。
2. 积压消息分发完成后，停止旧的消费者实例。
3. 为新 Topic 的每个分区启动一个消费者实例。
4. 积压消息消费完成后，恢复旧的消费者实例。
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-2.png)

## 16. 如何解决高性能读写数据的问题？

### 普通 I/O 和 NIO

- 普通 I/O 是基于字节的单向阻塞式传输。
- NIO 引入 FileChannel 全双工通道和 ByteBuffer 数据容器，ByteBuffer 可以精准控制写入磁盘的数据大小。
...
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-3.png)
...
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-4.png)
**内存映射 MMAP（零拷贝技术）** NIO 可以采用 MappedByteBuffer 数据容器进行读写数据，而 MappedByteBuffer#map() 底层采用了 MMAP 技术进行了文件映射。
...
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-5.png)
...
- 消费端随机读取 CommitLog 数据文件；由于程序局部性，后续加载的数据通常可以在 PageCache 中找到，从而避免缺页中断后再从磁盘加载。
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-6.png)
...
![img](../assets/RocketMQ%20%E9%9D%A2%E8%AF%95%E9%A2%98%E9%9B%86%E9%94%A6-7.png)
