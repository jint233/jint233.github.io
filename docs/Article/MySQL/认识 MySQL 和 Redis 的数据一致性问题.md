...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-1.jpg)

- **更新（修改/删除）数据时**，会有个时序问题：更新数据库与删除缓存的顺序（这个过程会发生数据不一致性问题）

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-2.jpg)
...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-3.jpg)

#### (2) 先更新数据库，再删除缓存

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-4.jpg)

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-5.jpg)

**解决策略：**  **a.消息队列+异步重试** 无论使用哪一种执行时序，可以在执行步骤 1 时，将步骤 2 的请求写入消息队列，当步骤 2 失败时，就可以使用重试策略，对失败操作进行 “补偿”。

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-6.jpg)
...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-7.jpg)

或者

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-8.jpg)
...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-9.jpg)
...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-10.jpg)

或者，在”先更新数据库，再删除缓存”方案下，“读写分离 + 主从库延迟”也会导致不一致：

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-11.jpg)
...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-12.jpg)
...
### **A. 无并发情况**![示意图](../assets/认识 MySQL 和 Redis 的数据一致性问题-13.jpg)

### **B. 高并发情况** 有四种场景会造成数据不一致

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-14.jpg)
...
![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-15.jpg)

其中，分布式锁的实现可以使用以下策略：

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-16.jpg)
...
![示意图](../assets/认识 MySQL 和 Redis 的数据一致性问题-17.jpg)

针对读写缓存时：同步直写，更新数据库+更新缓存

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-18.jpg)

针对只读缓存时：更新数据库+删除缓存

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-19.jpg)
...
1. 当业务遇到单个读热 key，通过增加副本来提高读能力，或用 hashtag 把 key 存多份在多个分片中。
2. 当业务遇到单个写热 key，需由业务拆分这个 key 的功能，属于设计不合理。
3. 当业务遇到热分片，即多个热 key 在同一个分片上导致单分片 CPU 高，可通过 hashtag 方式打散。

**[引自腾讯云技术分享]**

#### (2) 避免其他问题导致缓存服务器崩溃，进而间接导致数据一致性策略失效

缓存穿透、缓存击穿、缓存雪崩、机器故障等问题。

![图解](../assets/认识 MySQL 和 Redis 的数据一致性问题-20.jpg)
