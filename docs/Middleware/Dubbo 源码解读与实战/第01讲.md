...
![Drawing 0.png](assets/第01讲-01.png)

Dubbo 核心架构图

Dubbo 架构中的四大核心角色及其职责交互总结如下表所示：

| 架构角色       | 英文名称     | 核心职责与交互机制                                                     | 故障/宕机影响与容错                             |
|----------------|--------------|------------------------------------------------------------------------|-------------------------------------------------|
| **服务提供者** | **Provider** | 启动时向注册中心注册 URL 地址，监听并处理来自 Consumer 的 RPC 调用     | 单点宕机将由注册中心感知，推送给 Consumer 剔除  |
| **服务消费者** | **Consumer** | 启动时向注册中心订阅服务列表，缓存本地建立长连接，基于负载均衡发起调用 | 独立运行，重启自动重新订阅                      |
| **注册中心**   | **Registry** | 动态维护 Provider 地址列表，利用 ZooKeeper 心跳感知变更并推送通知      | 宕机不影响已建立的 Provider与Consumer直连长连接 |
| **监控中心**   | **Monitor**  | 异步统计服务调用次数与响应耗时，定时上报监控数据                       | 宕机完全不影响服务调用，仅丢失监控图表          |

### 搭建 Dubbo 源码环境

当然，要搭建 Dubbo 源码环境，你首先需要下载源码。这里你可以直接从官方仓库 [https://github.com/apache/dubbo](https://github.com/apache/dubbo)Fork 到自己的仓库，直接执行下面的命令去下载代码：

```shell
git clone jin@2333:xxxxxxxx/dubbo.git
```
...
![Drawing 2.png](assets/第01讲-02.png)
...
![Drawing 4.png](assets/第01讲-03.png)
...
![Drawing 5.png](assets/第01讲-04.png)
...
![Drawing 6.png](assets/第01讲-05.png)
...
![Drawing 7.png](assets/第01讲-06.png)
...
解压完成之后，进入 zookeeper-3.4.14 目录，复制 conf/zoo_sample.cfg 文件并重命名为 conf/zoo.cfg，之后执行如下命令就可以启动 ZooKeeper 了。
...
![Drawing 11.png](assets/第01讲-10.png)
...
![Drawing 12.png](assets/第01讲-11.png)
...
![Drawing 13.png](assets/第01讲-12.png)
