...
观察者模式根据数据传输的主动权与粒度，可以划分为**推模型**与**拉模型**：

| 协作模型                | 数据传递机制                                                       | 接口与参数设计                            | 优势特点                                       | 缺点与典型应用场景                                           |
|-------------------------|--------------------------------------------------------------------|-------------------------------------------|------------------------------------------------|--------------------------------------------------------------|
| **推模型 (Push Model)** | 被观察者主动将**全量数据或变化细节**推给所有观察者                 | `update(Object data)` 直接传递具体数据    | 观察者拿来即用，无需反向回调查询，系统彻底解耦 | 忽略观察者按需定制，信息可能冗余；适用于系统紧急维护全员广播 |
| **拉模型 (Pull Model)** | 被观察者仅发送**轻量变更通知（或自身引用）**，由观察者按需主动查询 | `update(Observable subject)` 传递自身引用 | 传输体极小，观察者具备最大自主权，按需拉取     | 观察者需依赖被观察者 Getter API；适用于 App 检查新版本提醒   |

推模型和拉模型更多的是语义和逻辑上的区别。我们上面的代码框架，从接口 `update(self, observer, object)` 上是可以同时支持推模型和拉模型的。推模型时，`object` 传具体数据；拉模型时，通过 `observer` 引用由观察者按需调用方法去取数据。
