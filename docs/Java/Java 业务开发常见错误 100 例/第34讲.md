# 34 加餐 4：分析定位 Java 问题，一定要用好这些工具（一）
...
![image](assets/第34讲-01.png)

接下来，我会与你介绍些常用的监控工具。你也可以先通过下面这张图了解下各种工具的基本作用：

![image](assets/第34讲-02.jpg)
...
![image](assets/第34讲-06.png)

点击面板的线程 Dump 按钮，可以查看线程瞬时的线程栈：

![image](assets/第34讲-07.png)
...
"main" #1 prio=5 os_prio=31 cpu=440.66ms elapsed=574.86s tid=0x00007ffdd9800000 nid=0x2803 waiting on condition  [0x0000700003849000]
  ...
  at java.lang.Thread.sleep(user@host.com/Native Method)
  at java.lang.Thread.sleep(user@host.com/Thread.java:339)
  at java.util.concurrent.TimeUnit.sleep(user@host.com/TimeUnit.java:446)
  ...
  at jdk.internal.reflect.NativeMethodAccessorImpl.invoke0(user@host.com/Native Method)
  at jdk.internal.reflect.NativeMethodAccessorImpl.invoke(user@host.com/NativeMethodAccessorImpl.java:62)
  at jdk.internal.reflect.DelegatingMethodAccessorImpl.invoke(user@host.com/DelegatingMethodAccessorImpl.java:43)
  at java.lang.reflect.Method.invoke(user@host.com/Method.java:566)
...
"Thread-1" #13 prio=5 os_prio=31 cpu=17851.77ms elapsed=574.41s tid=0x00007ffdda029000 nid=0x9803 waiting on condition  [0x000070000539d000]
  ...
  at java.lang.Thread.run(user@host.com/Thread.java:834)
...
```

抓取后可以使用类似 fastthread 这样的在线分析工具来分析线程栈。
...
我在加餐 3 中提到一条原则，“分析问题一定是需要依据的，靠猜是猜不出来的”。现在，我们就使用网络分析工具 Wireshark 来分析一下这个案例，眼见为实。

首先，我们可以在这里下载 Wireshark，启动后选择某个需要捕获的网卡。由于我们连接的是本地的 MySQL，因此选择 loopback 回环网卡：

![image](assets/第34讲-10.png)
...
你有没有想过，客户端是如何和 MySQL 进行认证的呢？你能否对照 MySQL 的文档，使用 Wireshark 观察分析这一过程呢？
