...
我们可以在 redis.conf 中使用 replica-priority 选项来设置竞选新主节点的优先级。它的默认值是 100，值越小权重越高。例如从节点 A 的 replica-priority 值为 100，从节点 B 的值为 50，从节点 C 的值为 5，那么在竞选时从节点 C 会作为新的主节点。

#### **新主节点竞选规则**

新主节点的竞选会排除不符合条件的从节点，然后在剩余的从节点中按照优先级来挑选。首先，存在以下条件的从节点会被排除：
