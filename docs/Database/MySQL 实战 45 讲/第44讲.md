...
在 [第 35 讲《join语句怎么优化》](./第35讲.md) 中，我在介绍 join 执行顺序的时候，用的都是 straight_join。
...
<figure markdown="span">
  ![图 1 两个 join 的查询结果](./assets/202305061610162.png)
  <figcaption>图 1 两个 join 的查询结果</figcaption>
</figure>
...
<figure markdown="span">
  ![图 2 Q1 的 explain 结果](./assets/202305061610163.png)
  <figcaption>图 2 Q1 的 explain 结果</figcaption>
</figure>
...
<figure markdown="span">
  ![图 3 left join -BNL 算法](./assets/202305061610164.jpeg)
  <figcaption>图 3 left join -BNL 算法</figcaption>
</figure>
...
<figure markdown="span">
  ![图 4 Q2 的 explain 结果](./assets/202305061610165.png)
  <figcaption>图 4 Q2 的 explain 结果</figcaption>
</figure>
...
<figure markdown="span">
  ![图 5 Q2 的改写结果](./assets/202305061610166.png)
  <figcaption>图 5 Q2 的改写结果</figcaption>
</figure>
...
<figure markdown="span">
  ![图 6 join 语句改写](./assets/202305061610167.png)
  <figcaption>图 6 join 语句改写</figcaption>
</figure>
...
我们知道，join 语句使用不同的算法，对语句的性能影响会很大。在[第 34 讲《到底可不可以使用join》](./第34讲.md) 的评论区中，@书策稠浊和 @朝夕心两位同学提了一个很不错的问题。
...
1. 在对被驱动表做全表扫描的时候，如果数据没有在 Buffer Pool 中，就需要等待这部分数据从磁盘读入；
  从磁盘读入数据到内存中，会影响正常业务的 Buffer Pool 命中率，而且这个算法天然会对被驱动表的数据做多次访问，更容易将这些数据页放到 Buffer Pool 的头部（请参考 [第 35 讲《join语句怎么优化》](./第35讲.md) 中的相关内容）；
...
在 [第 37 讲《什么时候会使用内部临时表》](./第37讲.md) 中，@老杨同志提了一个好问题：如果只需要去重，不需要执行聚合函数，distinct 和 group by 哪种效率高一些呢？
...
!!! Note "备注"
    这里你可以顺便复习一下第 37 篇文章中关于 group by 的相关内容。
...
除了性能问题，大家对细节的追问也很到位。在 [第 39 讲《自增主键为什么不是连续的？》](./第39讲.md) 评论区，@帽子掉了同学问到：在 binlog_format=statement 时，语句 A 先获取 id=1，然后语句 B 获取 id=2；接着语句 B 提交，写 binlog，然后语句 A 再写 binlog。这时候，如果 binlog 重放，是不是会发生语句 B 的 id 为 1，而语句 A 的 id 为 2 的不一致情况呢？
...
<figure markdown="span">
  ![图 7 insert 语句的 binlog](./assets/202305061610168.png)
  <figcaption>图 7 insert 语句的 binlog</figcaption>
</figure>
...
在 [第 08 讲《事务到底是隔离的还是不隔离的？》](./第08讲.md) 的评论区， @XD 同学提到一个问题：他查看了一下 innodb_trx，发现这个事务的 trx_id 是一个很大的数（281479535353408），而且似乎在同一个 session 中启动的会话得到的 trx_id 是保持不变的。当执行任何加写锁的语句后，trx_id 都会变成一个很小的数字（118378）。
