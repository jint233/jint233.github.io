...
在上面的示例代码中，我们定义了一个静态的 _\*instance_\* 类变量，用来存放 Singleton1 的对象，_\*new_\* 方法每次返回同一个__instance对象__（若未初始化，则进行初始化）。因为每一次通过 s = Singleton1() 的方式创建对象时，都会自动调用 _\*init_\* 方法来初始化实例对象；因此 _\*isFirstInit_\* 的作用就是确保只对 _\*instance_\* 对象进行一次初始化，故事剧情中的代码就是用这种方式实现的单例。
