...
NIO 也称新 IO 或者非阻塞 IO（Non-Blocking IO）。传统 IO 是面向输入 / 输出流编程的，而 NIO 是面向通道与缓冲区编程的。

传统 BIO 与 NIO 的核心差异如下表所示：

| 对比维度     | 传统 I/O (BIO)                             | 新 I/O (NIO)                                                          |
|--------------|--------------------------------------------|-----------------------------------------------------------------------|
| **处理单元** | 面向流（Stream），单向顺序流转             | 面向块与缓冲区（Buffer），通道（Channel）双向传输                     |
| **阻塞模式** | 阻塞式（Blocking），一个连接需配备一个线程 | 非阻塞式（Non-Blocking），基于 Reactor 多路复用（Selector）           |
| **数据传输** | 传统字节/字符数组传输                      | 内存映射 `MappedByteBuffer` / 零拷贝（`transferTo` / `transferFrom`） |
| **适用场景** | 连接数少的架构或本地文件读写               | 高并发、高连接数的网络服务（如 Netty、RPC 框架）                      |

NIO 的 3 个核心概念：Channel、Buffer、Selector。我们先来谈谈其中的两个。

### Channel（通道）

Channel 是对 IO 输入 / 输出系统的抽象，是 IO 源与目标之间的连接通道，NIO 的通道类似于传统 IO 中的各种 “流”。与 InputStream 和 OutputStream 不同的是，Channel 是双向的，既可以读，也可以写，且支持异步操作。这契合了操作系统的特性，比如 Linux 底层通道就是双向的。此外 Channel 还提供了 `map()` 方法，通过该方法可以将 “一块” 数据直接映射到内存中。因此 NIO 是面向块处理的，而传统 I/O 是面向流处理的。
...
![图解](../assets/Java IO 体系、线程模型大总结-1.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-2.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-3.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-4.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-5.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-6.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-7.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-10.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-11.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-12.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-13.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-14.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-15.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-16.png)
...
![图解](../assets/Java IO 体系、线程模型大总结-17.png)
