...
![2459010.png](assets/第24讲-01.png)
...
JVM 抛出“java.lang.OutOfMemoryError: GC overhead limit exceeded”错误，说明执行垃圾收集的时间比例太大而有效运算量太小。默认情况下，如果 GC 花费的时间超过 98%，并且回收的内存少于 2%，JVM 就会抛出这个错误。此时系统几乎所有资源都用于 GC，但回收效果很差。

![06cff4f1-b6a6-4cda-b7f5-8e8837f55c3a.png](assets/第24讲-02.png)
...
既然我们了解了问题所在，就可以考虑对应的解决办法。

**1. 解决程序启动时产生的 OutOfMemoryError**

在程序启动时，如果 PermGen 耗尽而产生 OutOfMemoryError 错误，增加 PermGen 的大小即可。修改 `-XX:MaxPermSize` 启动参数，例如：
...
和[上一章的 PermGen](http://blog.csdn.net/renfufei/article/details/77994177)类似，Metaspace 空间的使用量与 JVM 加载的 class 数量有很大关系。下面是一个简单的示例：
...
![7ff0ad95-3c2e-4246-badc-e007abf84978.png](assets/第24讲-03.png)

“java.lang.OutOfMemoryError: Unable to create new native thread”错误是 **程序创建的线程数量已达到上限值** 的异常信息。

### **原因分析**

JVM 向操作系统申请创建新的 native thread（原生线程）时，可能会碰到“java.lang.OutOfMemoryError: Unable to create new native thread”错误。如果底层操作系统创建新的 native thread 失败，JVM 就会抛出相应的 OutOfMemoryError。
...
有时可以修改系统限制来避开“Unable to create new native thread”问题。假如 JVM 受到用户空间（user space）文件数量的限制，可以在 Linux 上增大可用文件描述符的最大值（Linux 上一切都是文件描述符/FD）。
...
![40dddc4d-f192-40dc-9a1f-ab4df65d6f12.png](assets/第24讲-04.png)
...
![8cc643df-164f-4ee9-9142-7a27032418ec.png](assets/第24讲-05.png)

如果程序抛出“java.lang.OutOfMemoryError: Requested array size exceeds VM limit”错误，就说明程序想要创建的数组长度超过限制。

### **原因分析**

这个错误是在真正为数组分配内存之前，JVM 执行寻址检查时产生的：要分配的数据结构在该平台是否可以寻址（addressable）。这个错误比较少见。
...
![13abc504-c840-4264-a905-e9011159484f.png](assets/第24讲-06.png)
...
默认情况下，Linux kernel（内核）允许进程申请的内存量超过系统可用内存。这是因为在大多数情况下，很多进程申请了很多内存，但实际使用的量并没有那么多。
...
在 Linux 上（如最新稳定版的 Ubuntu）编译并执行以下示例代码：
