...
![avatar](assets/FsUnrCguMgMBVyC1ybspBDgGg5tz.png)
...
![avatar](assets/FtQwOqpvCjmT_9MCtqS2IA0FhkKx.png))
...
![avatar](assets/Ft9Rf5hiHOSF5h0P0dz-RUVpq2Fs.png) ![avatar](assets/Fju4lrcck9azmN8uV1jHsk2NYM5d.png)
...
![avatar](assets/Flzm1OeSsGAqzkPS_l_SL8OWo99t.png)) ![avatar](assets/FiyLMf6T0Q1Z-OAmAF7THSwsyIxG.png)
...
![avatar](assets/FlcotYqNLPbCvnibylrLt33enE1M.png)

Kubernetes 的 Master 包含四个主要的组件：API Server、Controller、Scheduler 以及 etcd。如下图所示：

![avatar](assets/FrFh4R6uKA2VDeclmHMZPxqTWbl_.png))
...
![avatar](assets/Fp29--O3Bo8y2VsC3C3SkwLILJ4-.png)
...
![avatar](assets/FsXK0GqTGIYAF9Vu5wndumZQlB7A.png)
...
![avatar](assets/FmFsx1ss6Nc01jifUhwW9NHW9W3_.png))
...
![avatar](assets/Fl18QUI1iine53dAy56-l4sErsLr.png)
...
![avatar](assets/FnD98YqsPIFP_5wKbyqD3Z7tKfOH.png))
...
![avatar](assets/Fqw4YYXdiHBS7TI2oFyCy55WVzP9.png)
...
![avatar](assets/FomVvOOmGfThgv1lQQ3d30d8v4aa.png)
...
![avatar](assets/Fp8roGITYZHjGDSiwrf3Q5zt7l1S.png)
...
![avatar](assets/FuUn6ZkhMD2b2UV6CY2jHYP8i450.png)
...
![avatar](assets/FlOUt7JuwbE0UBUz8NwA9vVYCuUl.png)

下一步我们利用 kubectl 来看一下这个集群中节选的状态，可以看到这个 master 的节点已经是 running 状态：

![avatar](assets/FlT6wcXrdtsvhAnoiNWVFRVx1iiM.png)

我们就以这个为节点，下面我们尝试去看一下现在集群中 Deployment 这个资源：

![avatar](assets/FggAaOvw-U4UXQS7NMrcO3ew02X_.png))

可以看到集群中没有任何的 Deployment，我们可以利用 watch 这个语义去看集群中 Deployment 这个资源的变化情况。

下面我们去做刚才想要的三个操作：第一个操作是去创建一个 Deployment。可以看到下面第一个图，这是一个 API 的 content，它的 kind 是 Deployment，name 是 nginx-deployment, 有图中它的 replicas 数目是 2，它的镜像版本是 1.7.9。

![avatar](assets/FrqXKjDLgyIqN_vTsIlWt1ImrvxZ.png)) ![avatar](assets/FpIjS0iNnlmx5GRpxdJFTcWlT1vo.png)

我们下面还是回到 kubectl 这个 commnd 来执行这次 Deployment 的真正的操作。我们可以看到一个简单的操作，就会去让 Deployment 不停地生成副本。

![avatar](assets/FlI67d3ZPrD37qNRdaNSrFE4xjfQ.png)
...
![avatar](assets/FjpIzqw7CKmfaIGlK8BhkfKhN4E3.png)

最后我们演示一下给 Deployment 做水平扩张，下载另一个 yaml 文件 deployment-scale.yaml，这里面的 replicas 数目已经从 2 改成了 4。

![avatar](assets/FmVI-oaSb0TJLObDGahsgdLZaOjV.png)
...
![avatar](assets/FuN37XsKiIT3WOYU5QNNIigMm1Ii.png)
...
![avatar](assets/Ft0eDGSed6lpc2TROivAJNV4wKpm.png)
