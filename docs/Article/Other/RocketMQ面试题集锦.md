...
![img](../assets/c75b8370-9ea4-11ea-9d24-cfb0df3065fc.png)
...
========================================
5. 创建一个 Topic 并且把积压消息的 Topic 下的分区中的消息分发到新建的那个 Topic 中的更多分区中，新建 Topic 的分区数是积压消息的 Topic 的分区的 10 倍（具体多少分区根据具体业务场景决定）。
6. 积压的消息分发完以后停掉旧的消费端实例。
7. 每一个新建 Topic 的分区都对应启动一个消费端实例。
8. 积压的消息都消费完后，再把旧的消费端实例恢复。
...
![img](../assets/79b95410-9ea6-11ea-853e-a34978cba4d6.png)
...
![img](../assets/c7c074e0-9ea6-11ea-853e-a34978cba4d6.png)
...
![img](../assets/251a5070-9ea7-11ea-bf38-950ba54cfedc.png) **mlock 系统调用** 当内存不足的时候可能发生内存 SWAP，读写消息的进程所使用的内存可能会被交换到 SWAP 空间，为了保证 RocketMQ 的吞吐量和读写消息低延迟，肯定希望尽可能使用物理内存，所以 RocketMQ 采用了 mlock 系统调用将读写消息的进程所使用的部分或者全部内存锁在了物理内存中，防止被交换到 SWAP 空间。**文件预热（内存预热）** mlock 系统调用未必会锁住读写消息进程所使用的物理内存，因为可能会有一些内存分页是写时复制的，所以 RocketMQ 在创建 MapperFile 的过程中，会将 Mmap 映射出的虚拟内存中随机写入一些值，防止内存被交换到 SWAP 空间。
