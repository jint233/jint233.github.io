...
这个问题的原因是这样的：看一下内核里的那段代码 `!(force && sig_kernel_only(sig))`：
...
- **Geek2014 用户的回答**：这时 C 是不会被回收的，只有等到 B 也被杀死，C 这个僵尸进程才会变成孤儿进程，被 init 进程收养，进而被 init 的 wait 机制清理掉。
- **莫名同学的回答**：C 应该不会被回收，`waitpid` 仅等待直接 children 的状态变化。为什么先进入僵尸状态而不是直接消失？留给父进程一次机会，查看子进程的 PID、终止状态（退出码、终止原因，比如是信号终止还是正常退出等）、资源使用信息。如果子进程直接消失，那么父进程没有机会掌握子进程的具体终止情况。一般情况下，程序逻辑可能会依据子进程的终止情况做出进一步处理：比如 Nginx Master 进程获知 Worker 进程异常退出，则重新拉起来一个 Worker 进程。
...
{
    // Ignore SIGTERM, and send SIGTERM to process itself.
    signal(SIGTERM, SIG_IGN);
    printf("Ignore SIGTERM\n\n");
    kill(0, SIGTERM);

    // Catch SIGTERM, and send SIGTERM to process itself.
    signal(SIGTERM, sig_handler);
    printf("Catch SIGTERM\n");
    kill(0, SIGTERM);

    // Default SIGTERM. In sig_handler, it sets SIGTERM handler back to default one.
...
故而，group1 和 group2 各分配到 2。把 group2 分到的 2 个 CPU 看作总量，再次分析 group3 和 group4。group3 和 group4 尝试使用的总量超过 2，所以按照 shares 比例分配，group3 使用 1/(1+3) \* 2 = 0.5，group4 使用 3/(1+3) \* 2 = 1.5。
...
A：通过 `memory.oom_control` 禁止 OOM 后，在容器中的进程不会发生 OOM，但是也无法申请出超过 `memory.limit_in_bytes` 的内存。
...
A：这里引用上邪忘川同学的实验结果。实验过程如下，结果是 lower1 目录中的文件覆盖了 lower2 中同名的文件，第一个挂载的目录优先级比较高。
...
Q：这是一道操作题，通过这个操作你可以再理解一下 blkio Cgroup 与 Buffered I/O 的关系。在 Cgroup V1 的环境里，我们在 blkio Cgroup V1 的例子基础上，把 fio 中“-direct=1”参数去除之后，再运行 fio，同时运行 iostat 查看实际写入磁盘的速率，确认 Cgroup V1 blkio 无法对 Buffered I/O 限速。
...
Q：在这一讲中，我们提到了 Linux 内核中的 `tcp_force_fast_retransmit()` 函数，那么你可以想想看，这个函数中的 `tp->reordering` 和内核参数 `/proc/sys/net/ipv4/tcp_reordering` 是什么关系？它们对数据包的重传会带来什么影响？
...
A：首先可以参考莫名同学的答案：关于思考题，想到一个比较笨拙的方法：gdb + qemu 调试内核。先进入虚拟机在某个内核函数上注册一个 kprobe，然后 gdb 远程调试内核，查看该内核函数的汇编指令（disass）是否被替换。应该有更简单的方法。
