...
在前面，我们讲述了 long 和 double 和其他基本类型不太一样，好像不具备原子性，这是什么原因造成的呢？官方文档对于上述问题的描述如下：
...
此时你可能会有疑问：如果之前对于上述问题不是很了解，在开发过程中没有给 long 和 double 加 volatile，好像也没有出现过问题？而且在以后的开发过程中，是不是必须给 long 和 double 加 volatile 才安全？
