...
<figure markdown="span">
  ![Spring 家族技术体系](./assets/第01讲-02.png)
  <figcaption>Spring 家族技术体系（来自 Spring 官网）</figcaption>
</figure>

从图中可以看到，这里罗列了 Spring 框架的七大核心技术体系，分别是微服务架构、响应式编程、云原生、Web 应用、Serverless 架构、事件驱动以及批处理。

Spring 7 大核心技术体系的职责定位与应用场景总结如下表所示：

| 技术体系       | 核心框架 / 代表          | 核心解决的问题与技术优势                   | 典型落地工程场景                       |
|----------------|--------------------------|--------------------------------------------|----------------------------------------|
| **Web 应用**   | Spring Boot / Spring MVC | 约定优于配置，极速构建 RESTful HTTP 服务   | 传统 MVC 架构、单体应用、标准 Web 接口 |
| **微服务架构** | Spring Cloud             | 解决服务注册发现、API 网关、配置中心与熔断 | 复杂分布式微服务系统架构               |
| **响应式编程** | Spring WebFlux           | 异步非阻塞 I/O，高吞吐响应式数据流处理     | 高并发、I/O 密集型、实时流处理         |
| **事件驱动**   | Spring Cloud Stream      | 屏蔽底层消息队列异构细节，统一消息收发 API | 基于 Kafka/RabbitMQ 的解耦消息传递     |
| **批处理**     | Spring Batch             | 提供健壮的批处理作业管理、事物与重试跟踪   | 离线大批量数据清洗、日终结算报表       |
| **云原生**     | Spring Cloud K8s         | 结合 Docker/K8s 容器编排实现弹性伸缩       | 容器化部署、Kubernetes 原生集成        |
| **Serverless** | Spring Cloud Function    | 函数即服务 (FaaS) 抽象，屏蔽云厂商差异     | 事件驱动型云函数、轻量逻辑处理         |
...
<figure markdown="span">
  ![Spring Framework 整体架构图](./assets/第01讲-03.png)
  <figcaption>Spring Framework 整体架构图</figcaption>
</figure>

Spring 从诞生之初就被认为是一种容器，上图中的“核心容器”部分就包含了一个容器所应该具备的核心功能，包括容器中基于依赖注入机制的 JavaBean 处理、面向切面 AOP、上下文 Context 及 Spring 自身所提供的表达式工具等一些辅助功能。
...
<figure markdown="span">
  ![Spring Boot 整体架构图](./assets/第01讲-04.png)
  <figcaption>Spring Boot 整体架构图</figcaption>
</figure>
...
让我们先来感受一下使用 Spring Boot 开发一个 RESTful 风格 的 HTTP 端点所需要做的编码工作，如下所示：
...
通过前面的描述，我们已经发现 Spring Boot 具备了传统 Spring 框架所不具备的功能特性，即支持运行期内嵌容器（包含 Tomcat、Jetty 等支持 Servlet 规范的多款传统 Web 容器）。而在最新的 Spring Boot 2.X 版本中，还提供了对 Netty 以及集成 Servlet 3.1+ 的非阻塞式容器的支持。基于运行期内嵌容器机制，开发人员想要启动 Web 服务只需要使用一行 java –jar 命令就可以了。
...
<figure markdown="span">
  ![Spring Cloud 与微服务整体架构图（来自 Spring 官网）](./assets/第01讲-05.png)
  <figcaption>Spring Cloud 与微服务整体架构图（来自 Spring 官网）</figcaption>
</figure>

技术组件的完备性是 Spring Cloud 框架的主要优势，它集成了业界一大批知名的微服务开发组件。Spring Cloud 的核心组件如下图所示：

<figure markdown="span">
  ![Spring Cloud 核心功能组件](./assets/第01讲-06.png)
  <figcaption>Spring Cloud 核心功能组件</figcaption>
</figure>
...
<figure markdown="span">
  ![响应式编程技术栈与 Servlet 技术栈之间的对比图](./assets/第01讲-08.png)
  <figcaption>响应式编程技术栈与 Servlet 技术栈之间的对比图（来自 Spring 官网）</figcaption>
</figure>
...
![在基于 Spring Boot 以及 Spring Cloud 的应用程序中，Spring WebFlux 和 Spring MVC 可以混合进行使用。](assets/第01讲-01.png){style="width:60%;max-width:600px"}
