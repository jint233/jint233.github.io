...
本节将使用图解的方式来阐述 RocketMQ 默认提供的消息消费队列负载机制。**AllocateMessageQueueAveragely** 平均连续分配算法。主要的特点是一个消费者分配的消息队列是连续的

![3](assets/第09讲-03.png) **AllocateMessageQueueAveragelyByCircle** 平均轮流分配算法，其分配示例图如下：

![4](assets/第09讲-04.png) **AllocateMachineRoomNearby** 机房内优先就近分配。其分配示例图如下：
