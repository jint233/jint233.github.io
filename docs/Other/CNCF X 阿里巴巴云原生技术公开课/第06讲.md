...
![img](assets/b7c97640-d2c3-11e9-a365-2956af74956a.png)
...
![avatar](assets/FqRZXqnvbkypbT-ARZ9EuciekCqD.png)
...
![avatar](assets/Fu2ivUsTaLUzVrDBu2futpWLaF1t.png)
...
![avatar](assets/Fs4OqN5_81-gMNZc4uNCtlVfAvbI.png))
...
![avatar](assets/FqPlCtPscJEM23b_Q-AjWN9FtiPT.png))
...
接下来我们可以看一下，如何对一个给定的 Deployment 更新它所有 Pod 的镜像版本呢？这里我们可以执行一个 kubectl 命令： **kubectl set image deployment.v1.apps/nginx-deployment nginx=nginx:1.9.1**
...
![avatar](assets/FkERYt8vsgt2u6-pHkJ9mnCHR6MM.png)
...
![avatar](assets/FkjjEs6sM54eWZN_7alEPntwKJdt.png))
...
![avatar](assets/Fl9CQjw6K_EzMe27iIdxhFpnck3O.png))
...
![avatar](assets/Fun-rY_7MbND5PC1obeqwxp1EZaR.png))

首先创建对应的 deployment。可以看到 deployment 中的 desired、current、up-to-date 以及 available 已经都达到了可用的期望状态。

![avatar](assets/FuSh7BFpVmPc66eS61Jf7tRJVFW7.png)

### Deployment 的结构

这里看到 spec 中的 replicas 是三个，selector 以及 template labels 中定义的标签都是 app：nginx，spec 中的 image 是我们期望的 nginx: 1.7.9；status 中的 available.replicas，readReplicas 以及 updatedReplicas 都是 3 个。

![avatar](assets/Fr7_fNTKJdbEARnEChYc7VyQ0HYu.png))
...
![avatar](assets/FiM92jx0nQVOikobcBbEnSNglgAO.png)

### 更新升级

当前只有最新版本的 replicaset，那么现在尝试对 deployment 做一次升级。

![avatar](assets/Ft2Q8l7S2xNRd8PfBxuwckqnGFXz.png)

“kubectl set image”这个操作命令，后面接 “deployment”，加 deployment.name，最后指定容器名，以及我们期望升级的镜像版本。

![avatar](assets/FgOs2Oj0b5VSYLQnd_BK2QSn7bLv.png))

接下来我们看下 deployment 中的 template 中的 image 已经更新为 1.9.1。

![avatar](assets/FgFsNXZdE74hsTGSjYawyQnJl4D2.png)

这个时候我们再 get pod 看一下状态。

![avatar](assets/FsIn9W6sH4_OVMXUR6VfTX_AwOoD.png))

三个 pod 已经升级为新版本，pod 名字中的 pod-template-hash 也已更新。

![avatar](assets/FhAamAvv0WT8fxmVODrO5gHeMRaa.png)
...
![avatar](assets/FpiPmioOOxkeXk7Ve-7ro7RmkFGS.png) ****
...
![avatar](assets/Fn-kTF85gQEi2Vkk8Jvqhyz2PgdI.png) ![avatar](assets/FtV6gw2MsQflGn8jprEnRwn8cTmg.png)
...
![avatar](assets/FiS6JI8v_MMVSX0e_PeKKLAjuszH.png))

我们最后再 get pod 看一下：

![avatar](assets/FtkkvKGGWYAMDqK6d7Icx2HtL2xI.png)
...
![avatar](assets/FuCB_FCHGkMRcZR1RE6EoxcT8IlH.png))
...
![avatar](assets/FpxE-4XAdA5wn4J97zkPKBJWkPgL.png)
...
![avatar](assets/Fr-7FZjePgouiaG6ism-GSMLJl_q.png))
...
![avatar](assets/FimypfAsafXJ_EL3iBSPjVbUelVa.png))
...
![avatar](assets/FmZKar-wXQtxLH9-rT4BZmZULMDw.png)
...
![avatar](assets/Fv9lYQXHc3Kh6d7VgdlAgDcXdGyh.png)
...
![avatar](assets/FiM7kHLUWa2eO47ePovKQtAkeLdA.png)
...
![avatar](assets/FsnmWASNbiP8rP7uWBJ9o9J7uGFI.png)
