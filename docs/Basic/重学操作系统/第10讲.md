...
再比如，RedHat 收购了 JBoss，把 JBoss 改名为 WildFly。 像 WildFly 这种工具更多是面向企业级，比如没有大量研发团队的企业会更倾向使用成熟的技术。RedHat 公司也有自己的 Linux，就叫作 RedHat。RedHat 系比较重要的 Linux 有 RedHat/Fedora 等。
...
关于`yum`的具体用法，你可以使用 man 工具进行学习。**apt**
...
![系统调用](assets/第10讲-06.png)

如果我们想精确查找一个叫作`mysql-server`的包，可以用`apt list`。

![软中断](assets/第10讲-07.png)
...
![上下文恢复](assets/第10讲-10.png)

可以像我这样使用`cd`先切换到家目录。

- 第二步：解压。我们解压下载好的`nginx`源码包。

![中断嵌套](assets/第10讲-11.png)
...
`-x`代表 extract（提取）。-z 代表`gzip`，也就是解压`gz`类型的文件。`-v`代表 verbose（显示细节），如果你不输入`-v`，就不会打印解压过程了。`-f`代表 file，这里指的是要操作文件，而不是磁带。 所以`tar`解压通常带有`x`和`f`，打包通常是`c`就是 create 的意思。

- 第三步：配置和解决依赖。解压完，我们进入`nginx`的目录看一看。 如下图所示：

![中断屏蔽](assets/第10讲-12.png)
...
![中断控制器](assets/第10讲-15.png)

安装完成之后，再执行`./configure`，如下图所示：

![中断处理程序](assets/第10讲-16.png)

我们看到配置程序开始执行。但是最终报了一个错误，如下图所示：

![中断注册](assets/第10讲-17.png)
