...
Redis 集群模式至少需要三个主节点，作为举例，本文搭建一个 3 主 3 备的精简集群，麻雀虽小，五脏俱全。主备关系如下图所示，其中 M 代表 Master 节点，S 代表 Slave 节点，A-M 和 A-S 为一对主备节点。

![第05讲-01](assets/第05讲-01.png)

按照上图所示的拓扑结构，如果节点 1 故障下线，那么节点 2 上的 A-S 将升主为 A-M，Redis 3 节点集群仍可用，如下图所示：

![第05讲-02](assets/第05讲-02.png)

特别说明：事实上，Redis 集群节点间是两两互通的，如下图所示，上面作为示意图，进行了适当简化。

![第05讲-03](assets/第05讲-03.png)
...
**config.c**：SSL 双向认证通信涉及的 keyStore 和 trustStore 密码密文、路径等信息可由 Redis 的配置文件 `redis.conf` 提供，如此，我们需要修改加载配置文件的源码（config.c->loadServerConfigFromString(char \*config)），部分修改如下：

![第05讲-14](assets/第05讲-14.png)

**redis.h**：Redis 的客户端（redisClient）和服务端（redisServer）都需要适配，部分代码如下：

![第05讲-15](assets/第05讲-15.png)

![第05讲-16](assets/第05讲-16.png)

**hiredis.h**：修改创建连接的原函数：

![第05讲-17](assets/第05讲-17.png)

**anet.h**：定义 SSL 通信涉及的一些函数（实现在 anet.c 中）：

![第05讲-18](assets/第05讲-18.png)
...
![第05讲-27](assets/第05讲-27.png)

上面代码只从一个节点的视角进行了检查，完整的代码将遍历所有节点，从所有节点的视角分别检查。

**Redis 集群创建** 大家可参考第二节 "2. 基于 Lettuce 创建 Redis 集群" 中的内容。
