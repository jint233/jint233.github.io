...
![第18讲 示意图](assets/第18讲-03.jpg)

attach 启动 Java 进程后，可以在 **Class Browser** 菜单中查看加载的所有类信息。我们在搜索框中输入 **InvokeDemo**，找到要查看的类。

![第18讲 示意图](assets/第18讲-04.jpg) **@** 符号后面的，就是具体的内存地址，我们可以复制一个，然后在 **Inspector** 视图中查看具体的属性，可以 **大体** 认为这就是类在方法区的具体存储。

![第18讲 示意图](assets/第18讲-05.jpg)

在 Inspector 视图中，我们找到方法相关的属性 **_methods**，可惜它无法点开，也无法查看。

![第18讲 示意图](assets/第18讲-06.jpg)
...
有时候在写一些 Python 脚本或者 JS 脚本时，特别羡慕这些动态语言。如果把查找目标方法的决定权，从虚拟机转嫁给用户代码，我们就会有更高的自由度。
...
![第18讲 示意图](assets/第18讲-11.jpg) **句柄类型**（MethodType）是我们对方法的具体描述，配合方法名称，能够定位到一类函数。访问方法句柄和调用原来的指令基本一致，但它的调用异常，包括一些权限检查，在运行时才能被发现。
