...
由此可见，Netty 内存池和 Recycler 对象池优化的核心目标都是为了减少资源分配的开销，避免大量朝生夕灭的对象造成严重的内存消耗和 GC 压力。关于内存池和对象池的原理可以复习下之前课程[《举一反三：Netty 高性能内存管理设计(上/下)》](../第13讲)[《轻量级对象回收站：Recycler 对象池技术解析》](../第15讲)，值得我们反复消化理解。
