...
我们现在用的 Intel CPU，通常都是多核的。每一个 CPU 核里面，都有独立属于自己的 L1、L2 的 Cache，然后再有多个 CPU 核共用的 L3 的 Cache、主内存。
...
![多核 CPU 缓存层级](assets/第38讲-01.jpeg)
