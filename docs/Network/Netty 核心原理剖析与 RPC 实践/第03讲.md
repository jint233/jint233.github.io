...
- `TCP_NODELAY`：Netty 默认是 `true`，表示立即发送数据。如果设置为 `false`，表示启用 Nagle 算法；该算法会将 TCP 网络数据包累积到一定量才发送。虽然可以减少报文发送数量，但会造成一定的数据延迟。Netty 为最小化数据传输延迟，默认禁用了 Nagle 算法。
...
bind() 方法涉及的细节比较多，我们将在[《源码篇：从 Linux 出发深入剖析服务端启动流程》](./第17讲.md)课程中做详细地解析，在这里就先不做展开了。
