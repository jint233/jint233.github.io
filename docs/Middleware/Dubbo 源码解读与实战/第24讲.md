...
![Drawing 0.png](assets/第24讲-01.png)
...
![Lark20201016-164613.png](assets/第24讲-02.png)
...
![Drawing 2.png](assets/第24讲-03.png)
...
![Drawing 3.png](assets/第24讲-04.png)
...
![Drawing 4.png](assets/第24讲-05.png)
...
![Drawing 5.png](assets/第24讲-06.png)
...
![Drawing 6.png](assets/第24讲-07.png)

RpcInvocation 继承关系图

下面是 RpcInvocation 中的核心字段，通过读写这些字段即可实现 Invocation 接口的全部方法。

- targetServiceUniqueName（String 类型）：要调用的唯一服务名称，其实就是 ServiceKey，即 interface/group:version 三部分构成的字符串。
- methodName（String 类型）：调用的目标方法名称。
- serviceName（String 类型）：调用的目标服务名称，示例中就是 org.apache.dubbo.demo.DemoService。
- parameterTypes（Class\<?>[]类型）：记录了目标方法的全部参数类型。
- parameterTypesDesc（String 类型）：参数列表签名。
...
- invokeMode（InvokeMode 类型）：此次调用的模式，分为 SYNC、ASYNC 和 FUTURE 三类。
...
![Drawing 7.png](assets/第24讲-08.png)
...
在 DubboProtocol.optimizeSerialization() 方法中，就会获取该优化器中注册的类，通知底层的序列化算法进行优化，序列化的性能将会被大大提升。当然，在进行序列化的时候，难免会级联到很多 Java 内部的类（例如，数组、各种集合类型等），Kryo、FST 等序列化算法已经自动将 JDK 中的常用类进行了注册，所以无须重复注册它们。
...
![Drawing 8.png](assets/第24讲-09.png)

getRegisteredClasses() 方法的调用位置

按照 Dubbo 官方文档的说法，即使不注册任何类进行优化，Kryo 和 FST 的性能依然普遍优于 Hessian2 和 Dubbo 序列化。
