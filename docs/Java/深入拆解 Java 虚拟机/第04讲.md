...
前不久在写代码的时候，我不小心踩到一个可变长参数的坑。你或许已经猜到了，它正是可变长参数方法的重载造成的。（注：官方文档建议避免重载可变长参数方法，见 [参考1](https://docs.oracle.com/javase/8/docs/technotes/guides/language/varargs.html) 的最后一段。）
...
对于 Java 语言中重写而 Java 虚拟机中非重写的情况，编译器会通过生成桥接方法 [参考2](https://docs.oracle.com/javase/tutorial/java/generics/bridgeMethods.html) 来实现 Java 中的重写语义。
...
唯一的例外在于，如果虚拟机能够确定目标方法有且仅有一个，比如说目标方法被标记为 final[参考3](https://wiki.openjdk.java.net/display/HotSpot/VirtualCalls) [参考4](https://wiki.openjdk.java.net/display/HotSpot/VirtualCalls)，那么它可以不通过动态类型，直接确定目标方法。
...
2. 范型参数类型造成的方法参数类型不一致：
    ...
    ```
- [https://docs.oracle.com/javase/8/docs/technotes/guides/language/varargs.html](https://docs.oracle.com/javase/8/docs/technotes/guides/language/varargs.html)
