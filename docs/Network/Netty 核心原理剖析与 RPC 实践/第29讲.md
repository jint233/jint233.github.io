...
在[《源码篇：解密 Netty Reactor 的线程模型》](./第18讲.md)课程中，我们介绍了 NioEventLoop 的核心原理。NioEventLoop 通过核心方法 select() 不断轮询注册的 I/O 事件，Netty 提供了选择策略 SelectStrategy 对象，它用于控制 select 循环行为，包含 CONTINUE、SELECT、BUSY_WAIT 三种策略。SelectStrategy 对象的默认实现就是使用的饿汉式单例，源码如下：
...
![Drawing 0.png](assets/Cip5yGAKfLmANJ_0AAZUvQP4FxQ293.png){style="width:100%;max-width:800px"}

对于 Netty 中责任链模式的实现，也遵循了责任链模式的四个基本要素：

### 责任处理器接口

ChannelHandler 对应的就是责任处理器接口，ChannelHandler 有两个重要的子接口：ChannelInboundHandler 和 ChannelOutboundHandler，分别拦截入站和出站的各种 I/O 事件。
...
![Drawing 1.png](assets/CgpVE2AKfMSAehuyAA0_w5_fB-c556.png){style="width:100%;max-width:600px"}
...
![Drawing 2.png](assets/Cip5yGAKfM6AHTnGAA43hnlCx54417.png){style="width:100%;max-width:600px"}
...
Netty 中观察者模式的运用非常多，但是并没有以上示例代码这么直观，我们平时经常使用的 ChannelFuture#addListener 接口就是观察者模式的实现。我们先来看下 ChannelFuture 使用的示例：
