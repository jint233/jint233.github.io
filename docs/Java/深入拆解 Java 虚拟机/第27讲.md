...
这些结构之间也有从属关系，如上面这段代码所示 ([出处](http://hannesdorfmann.com/annotation-processing/annotationprocessing101)）。我们可以通过`TypeElement.getEnclosedElements`方法，获得上面这段代码中`Foo`类的字段、构造器以及方法。

我们也可以通过`ExecutableElement.getParameters`方法，获得`setA`方法的参数。具体这些`Element`类都有哪些 API，你可以参考它们的 [Javadoc](https://docs.oracle.com/javase/10/docs/api/javax/lang/model/element/package-summary.html)。
...
如果你感兴趣的话，可以参考 [Project Lombok](https://projectlombok.org/)。这个项目自定义了一系列注解，并根据注解的内容来修改已有的源代码。例如它提供了`@Getter`和`@Setter`注解，能够为程序自动添加`getter`以及`setter`方法。有关对使用内部 API 的讨论，你可以参考 [这篇博客](http://notatube.blogspot.com/2010/11/project-lombok-trick-explained.html)，以及 [Lombok 的回应](http://jnb.ociweb.com/jnb/jnbJan2010.html#controversy)。
