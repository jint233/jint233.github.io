...
```plaintext
[root@ ~]# curl -sSL https://dl.k8s.io/release/stable.txt
v1.11.3
```
...
```plaintext
[root@ ~]# curl -sSL https://dl.k8s.io/release/v1.11.3/bin/linux/amd64/kubeadm > /usr/bin/kubeadm
[root@ ~]# chmod a+rx /usr/bin/kubeadm
...
![第05讲 图1](assets/第05讲-01.webp)

终端下可使用如下方式下载：

```plaintext
[root@ tmp]# wget -q https://dl.k8s.io/v1.11.3/kubernetes-server-linux-amd64.tar.gz
```
...
```plaintext
[root@ tmp]# echo 'e49d0db1791555d73add107d2110d54487df538b35b9dde0c5590ac4c5e9e039 kubernetes-server-linux-amd64.tar.gz' | sha256sum -c -
kubernetes-server-linux-amd64.tar.gz: 确定
[root@ tmp]# tar -zxf kubernetes-server-linux-amd64.tar.gz
[root@ tmp]# ls kubernetes
addons  kubernetes-src.tar.gz  LICENSES  server
[root@ tmp]# ls kubernetes/server/bin/ | grep -E 'kubeadm|kubelet|kubectl'
kubeadm
kubectl
kubelet
```
...
```shell
[root@ tmp]# mv kubernetes/server/bin/kube{adm,ctl,let} /usr/bin/
[root@ tmp]# ls /usr/bin/kube*
/usr/bin/kubeadm  /usr/bin/kubectl  /usr/bin/kubelet
[root@ tmp]# kubeadm version
kubeadm version: &version.Info{Major:"1", Minor:"11", GitVersion:"v1.11.3", GitCommit:"a4529464e4629c21224b3d52edfe0ea91b072862", GitTreeState:"clean", BuildDate:"2018-09-09T17:59:42Z", GoVersion:"go1.10.3", Compiler:"gc", Platform:"linux/amd64"}
[root@ tmp]# kubectl version --client
Client Version: version.Info{Major:"1", Minor:"11", GitVersion:"v1.11.3", GitCommit:"a4529464e4629c21224b3d52edfe0ea91b072862", GitTreeState:"clean", BuildDate:"2018-09-09T18:02:47Z", GoVersion:"go1.10.3", Compiler:"gc", Platform:"linux/amd64"}
[root@ tmp]# kubelet --version
Kubernetes v1.11.3
```
...
```shell
[root@ tmp]# cat <<'EOF' > /etc/systemd/system/kubelet.service
[Unit]
...
EOF
[root@ tmp]# mkdir -p /etc/systemd/system/kubelet.service.d
[root@ tmp]# cat <<'EOF' > /etc/systemd/system/kubelet.service.d/kubeadm.conf
[Service]
...
EOF
[root@ tmp]# systemctl enable kubelet
...
![第05讲 图2](assets/第05讲-02.webp)

```plaintext
[root@ ~]# wget https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.11.1/crictl-v1.11.1-linux-amd64.tar.gz
[root@ ~]# echo 'ccf83574556793ceb01717dc91c66b70f183c60c2bbec70283939aae8fdef768 crictl-v1.11.1-linux-amd64.tar.gz' | sha256sum -c -
crictl-v1.11.1-linux-amd64.tar.gz: 确定
[root@ ~]# tar zxvf crictl-v1.11.1-linux-amd64.tar.gz
[root@ ~]# mv crictl /usr/bin/
```
...
```go
func portForward(client libdocker.Interface, podSandboxID string, podName string, podNamespace string, port int32, stream io.ReadWriteCloser) error {
    // 省略了和 socat 无关的代码
...
```plaintext
[root@ ~]# kubeadm config images pull
...
```plaintext
  [root@ ~]# kubectl --kubeconfig /etc/kubernetes/admin.conf get nodes                                                
  NAME      STATUS     ROLES     AGE       VERSION
  master    NotReady   master    13h       v1.11.3
  [root@ ~]#
  [root@ ~]# KUBECONFIG=/etc/kubernetes/admin.conf kubectl get nodes                                                  
  NAME      STATUS     ROLES     AGE       VERSION
  master    NotReady   master    13h       v1.11.3
```
...
[root@ ~]# kubectl apply -f https://raw.githubusercontent.com/coreos/flannel/v0.10.0/Documentation/kube-flannel.yml
...
```shell
[root@ ~]# kubeadm join 202.182.112.120:6443 --token t14kzc.vjurhx5k98dpzqdc --discovery-token-ca-cert-hash sha256:d64f7ce1af9f9c0c73d2d737fd0095456ad98a2816cb5527d55f984c8aa8a762
[preflight] running pre-flight checks
