...
1. **Store** ：存储一对 key-value 值。
   1. **Load** ：根据 key 获取对应的 value 值，并且可以判断 key 是否存在。
   2. **LoadOrStore** ：如果 key 对应的 value 存在，则返回该 value；如果不存在，存储相应的 value。
   3. **Delete** ：删除一个 key-value 键值对。
   4. **Range** ：循环迭代 sync.Map，效果与 for range 一样。
...
![Drawing 3.png](assets/第10讲-02.png)
