...
从内存状态持久化成 RDB 文件时，会对 key 进行过期检查。过期的键不会被保存到新的 RDB 文件中，因此 Redis 中的过期键不会对生成新 RDB 文件产生任何影响。
...
当 Redis 以 AOF 模式持久化时，如果数据库中的某个过期键还没被删除，AOF 文件会保留此过期键；当此过期键被删除后，Redis 会向 AOF 文件追加一条 DEL 命令来显式删除该键值。
