...
![Drawing 1.png](assets/第07讲-01.png)
...
![Drawing 3.png](assets/第07讲-02.png)
...
MySQL 5.6 版本开始支持通过插件 Memcached Plugin，以 KV 方式访问表，这时可以将 MySQL 视作一个 Memcached KV 数据库。
...
执行完上述操作后，会新增一个库 innodb_memcache，里面的表 containers 就是需要配置的 KV 映射表。如果业务常见的主键查询 SQL 如下，其中列 user_id 是主键：
...
![Drawing 5.png](assets/第07讲-03.png)

从测试结果可以看到，**基于 Memcached 的 KV 访问方式比传统的 SQL 方式要快 54.33%**，而且 CPU 的开销反而还要低 20%。
...
![Drawing 6.png](assets/第07讲-04.png)
...
![Drawing 8.png](assets/第07讲-05.png)
