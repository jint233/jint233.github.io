...
本篇整理 RocketMQ 集群运维中常用的管理命令，并说明各参数和输出字段的含义。示例命令基于 RocketMQ 4.7.0，实际输出可能因版本和集群配置而有所不同。

## 集群命令汇总

### 集群列表

使用 `clusterList` 查看集群节点、主从角色以及各节点的写入和读取 TPS：
...
| `InTPS` | 节点每秒写入的消息数量 |
| `OutTPS` | 节点每秒读出的消息数量 |
| `PCWait` | pageCacheLockTimeMills（消息落盘时的加锁耗时） |
| `Hour` | 磁盘存储多久的有效消息（当前时间与磁盘存储最早的一条消息时间戳的差值） |
| `SPACE` | 磁盘已使用的占比 |

#### 集群中资源吞吐

使用 `statsAll` 查看集群中各主题和消费组的实时吞吐情况：
...
使用 `updateTopic` 创建主题，也可以修改主题的队列数量、权限等配置：
...
使用 `topicRoute` 查看 Topic 的路由信息，包括队列所在的 Broker 及其所属集群：
...
使用 `topicStatus` 查看主题状态，包括最小偏移量、最大偏移量和最新更新时间：
...
使用 `updateTopicPerm` 修改主题权限：写权限为 2、读权限为 4、读写权限为 6。下面将主题从读写权限改为写权限：
...
使用 `updateSubGroup` 创建消费组，成功后会返回消费组配置：
...
使用 `consumerStatus` 查看消费者状态，包括客户端版本和消费组名称：
...
RocketMQLog:WARN No appenders could be found for logger (io.netty.util.internal.PlatformDependent0).
RocketMQLog:WARN Please initialize the logger system properly.
001  root@ V4_7_0               1595768036031/root@
002  root@ V4_7_0               1595768036031/root@
```
...
使用 `consumerProgress` 查看消费组在各订阅 Queue 上的消费进度：
...
使用 `resetOffsetByTime` 将消费组位点重置到指定时间，从该时间点重新消费：
...
使用 `brokerStatus` 查看 Broker 的启动时间、版本、吞吐和磁盘等运行信息：
...
| `pageCacheLockTimeMills` | 消息存储时的加锁耗时 |
...
| `remainTransientStoreBufferNumbs` | 堆外可用缓存区数量，初始大小为 5，每个大小 1G，为在开启堆外内存传输时有效 |
...
使用 `getBrokerConfig` 获取 Broker 配置，下面仅展示命令和部分输出：
...
使用 `updateBrokerConfig` 热更新 Broker 配置，通常即时生效，无需重启 Broker 节点：
...
使用 `sendMsgStatus` 向指定 Broker 发送测试消息，检查 Broker 是否正常：
...
使用 `queryMsgById` 根据消息 ID 检索集群中的消息：
...
使用 `queryMsgByKey` 根据消息 Key 检索消息：
...
消息存储在 Broker 的 Queue 中，也可以通过 offset 检索消息：
...
使用 `queryMsgByUniqueKey` 根据 UniqueKey 检索消息：
