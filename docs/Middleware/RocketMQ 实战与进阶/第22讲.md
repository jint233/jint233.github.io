...
监控和业务同学反馈发送超时，而且频繁出现。具体现象如下图。**预热现象**![第22讲-配图3](assets/第22讲-03.jpg)

![第22讲-配图4](assets/第22讲-04.jpg)

说明：上图分别为开启预热时（`warmMapedFileEnable=true`）集群的发送 RT 监控、Broker 开启预热设置时的日志。**内存传输现象**![CPU 抖动](assets/第22讲-05.jpg)
