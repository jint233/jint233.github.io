# 20 容器安全（2）：在容器中，我不以 root 用户来运行程序可以吗？
...
[root@ /]# ls /mnt/shadow -l
---------- 1 root root 586 Nov 26 13:47 /mnt/shadow
[root@ /]# echo "hello" >> /mnt/shadow
```
...
[root@ /]# id     ### 容器中的用户是root
uid=0(root) gid=0(root) groups=0(root)
[root@ /]# sleep 3600   ### 在容器中启动一个sleep 进程
# ps -ef |grep sleep   ###在宿主机上查看容器sleep进程对应的用户
redhat   29433 29410  0 05:14 pts/0    00:00:00 sleep 3600
```

目前 Docker 和 podman 都支持了 rootless container，Kubernetes 对 rootless container 支持的工作也在进行中。
