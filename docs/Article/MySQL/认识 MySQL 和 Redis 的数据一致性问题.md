...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-1.jpg)

- **更新（修改/删除）数据时**，会有个时序问题：更新数据库与删除缓存的顺序（这个过程会发生数据不一致性问题）

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-2.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-3.jpg)

#### (2) 先更新数据库，再删除缓存

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-4.jpg)

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-5.jpg)
**解决策略：**  **a.消息队列+异步重试** 无论使用哪一种执行时序，可以在执行步骤 1 时，将步骤 2 的请求写入消息队列，当步骤 2 失败时，就可以使用重试策略，对失败操作进行 “补偿”。

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-6.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-7.jpg)

或者

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-8.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-9.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-10.jpg)

或者，在”先更新数据库，再删除缓存”方案下，“读写分离 + 主从库延迟”也会导致不一致：

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-11.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-12.jpg)
...
### **A. 无并发情况**![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-13.jpg)

### **B. 高并发情况** 有四种场景会造成数据不一致

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-14.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-15.jpg)

其中，分布式锁的实现可以使用以下策略：

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-16.jpg)
...
### **2.4 小结**![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-17.jpg)

针对读写缓存时：同步直写，更新数据库+更新缓存

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-18.jpg)

针对只读缓存时：更新数据库+删除缓存

![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-19.jpg)
...
![img](../assets/认识 MySQL 和 Redis 的数据一致性问题-20.jpg)
**(3)方案选定的思路**
