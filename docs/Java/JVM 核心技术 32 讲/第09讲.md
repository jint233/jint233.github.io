...
Java 也 提供了类似的命令行工具，叫做 JPS，用于展示 Java 进程信息（列表）。
...
Usage:
    ...
    jmap [option] <executable <core>
        (连接到 core file)
    jmap [option] [@]<remote-IP-hostname>
        (连接到远程 debug 服务)
...
Usage:
    ...
    jstack [-m] [-l] [@]<remote server IP or hostname>
...
Usage:
    ...
    jinfo [option] <executable <core>
        (to connect to a core file)
    jinfo [option] [@]<remote-IP-hostname>
...
![56345767.png](assets/第09讲-01.png)
