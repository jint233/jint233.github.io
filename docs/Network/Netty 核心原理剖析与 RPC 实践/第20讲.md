...
![Drawing 0.png](assets/CgpVE1_qwuqAN-08AAkfe67UOIA904.png){style="width:100%;max-width:600px"}
...
![Drawing 1.png](assets/Ciqc1F_qwvCAauqfAAI07PytbZY507.png){style="width:100%;max-width:600px"}
...
![Drawing 2.png](assets/Ciqc1F_qwveAHxDEAASdhRTxzyk624.png){style="width:100%;max-width:800px"}
...
![图片 2.png](assets/CgqCHl_qyZGABbMMAACKf1C8HLE741.png){style="width:100%;max-width:600px"}
...
![Drawing 3.png](assets/Ciqc1F_qw1KAUXO0AAMZJ_Hk4dQ099.png){style="width:100%;max-width:400px"}

通过上面 FastThreadLocal 的内部结构图，我们对比下与 ThreadLocal 有哪些区别呢？FastThreadLocal 使用 Object 数组替代了 Entry 数组，Object\[0\] 存储的是一个 Set<FastThreadLocal<?>> 集合，从数组下标 1 开始都是直接存储的 value 数据，不再采用 ThreadLocal 的键值对形式进行存储。

假设现在我们有一批数据需要添加到数组中，分别为 value1、value2、value3、value4，对应的 FastThreadLocal 在初始化的时候生成的数组索引分别为 1、2、3、4。如下图所示。

![Drawing 4.png](assets/Ciqc1F_qw1qAYzsdAAEbsTk70Is389.png){style="width:100%;max-width:800px"}
...
![Drawing 5.png](assets/Ciqc1F_qw2WAV1UtAAWTkglpnjs396.png){style="width:100%;max-width:600px"}
...
1. FastThreadLocal 真的一定比 ThreadLocal 快吗？答案是不一定的，只有使用 FastThreadLocalThread 类型的线程才会更快，如果是普通线程反而会更慢。
