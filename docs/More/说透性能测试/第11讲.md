...
随着微服务的流行，链路监控越来越受重视。微服务架构是根据业务进行拆分，对外统一暴露 API 接口，而内部可能是分布式服务、分布式对象存储等，如图 1 所示。

![1shangchuan.png](assets/第11讲-01.png)
...
![图示](assets/第11讲-05.png)

图 3：服务部署图

![配图](assets/第11讲-06.png)
...
![配图](assets/第11讲-07.png)

（2）在打开的机器人详情页面点击添加按钮，如下图所示：

![配图](assets/第11讲-08.png)
...
![配图](assets/第11讲-09.png)

配置好之后我们可以看到设置报警的钉钉群“SkyWalking 钉钉报警”出现了报警机器人消息，如下图所示：

![配图](assets/第11讲-10.png)

我们可以用 Linux 命令行工具 curl 快速验证是否可以推送成功，curl 命令行示意如下：

```shell
[@ ~]# curl 'https://oapi.dingtalk.com/robot/send?access_token=xxxxxxx' -H 'CONTENT-TyPE: application/json' -d '{"msgtype": "text","text": {"content": "业务报警"}}'
{"errcode":0,"errmsg":"ok"}
```
...
![配图](assets/第11讲-11.png)

图 5：业务报错图

![配图](assets/第11讲-12.png)
