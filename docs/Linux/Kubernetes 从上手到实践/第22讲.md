...
- 验证
  ...
  与我们平时使用 NGINX 基本一致。 现在编辑本地的 HOSTS 文件绑定 Node 的 IP 与 `saythx.moelove.info` 这个域名。使用浏览器进行访问 `saythx.moelove.info:刚才 Controller 使用 NodePort 暴露服务时的端口`：

![img](assets/167dc55e62bc0a88.webp)
