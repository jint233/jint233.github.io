...
微内核架构具有同样的意义，它提供一种用来实现基于插件（Plugin）的内部代码库，允许我们在系统中添加新增的或者排斥包含在核心架构中的特定业务功能。同时，微内核架构提供了一种定制化和扩展性的核心引擎。通过提供插件，可以赋予核心架构各种各样的业务特性。所以，**针对微内核架构的整体扩展性，我们可以通过接入新的插件来实现机制的演进，而不需要对核心系统做任何的修改。**
...
ShardingSphere 中大量使用了微内核架构来解耦系统内核和各个组件之间的关联关系，我们基于解析引擎和配置中心给出了具体的实现案例。在学习这些案例时 **，重点在于掌握 ShardingSphere 中对 JDK 中 SPI 的封装机制。**
