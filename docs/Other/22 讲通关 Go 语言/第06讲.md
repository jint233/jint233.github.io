...
这就是面向接口的好处，只要定义和调用双方满足约定，就可以使用，而不用管具体实现。接口的实现者也可以更好地升级重构，而不会有任何影响，因为接口约定没有变。
...
```go
// 工厂函数，返回一个 error 接口，其实具体实现是 *errorString。
...
// ReadWriter 是 Reader 和 Writer 的组合。
