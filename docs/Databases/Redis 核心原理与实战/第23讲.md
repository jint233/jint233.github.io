...
我们可以使用 `config get maxmemory-policy` 命令来查看当前 Redis 的内存淘汰策略，命令如下：
...
Redis 使用的是一种近似 LRU 算法，目的是更好地节约内存。它会给现有数据结构添加一个额外字段，用于记录键值的最后一次访问时间；内存淘汰时使用随机采样的方式淘汰数据，默认随机取 5 个值（此值可配置），然后淘汰最久没有使用的那个。

**3. LRU 算法缺点**

LRU 算法的缺点是：很久没有使用的键值如果最近被访问了一次，就不会被淘汰，即使它是使用次数最少的缓存。因此 Redis 4.0 之后引入了 LFU 算法，下面我们一起来看。
