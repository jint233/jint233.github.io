...
![Lark20201229-160604.png](assets/第45讲-01.png)
...
![Lark20201229-160606.png](assets/第45讲-02.png)
...
![Drawing 2.png](assets/第45讲-03.png)
...
![Drawing 3.png](assets/第45讲-04.png)
...
if (NodeChildrenChanged.equals(eventType) || NodeDataChanged.equals(eventType)) {
    ...
    // 调用 dispatchServiceInstancesChangedEvent()方法，分发 ServiceInstancesChangedEvent 事件
...
// 该线程池用于串行调用被触发的 EventListener，也就是 direct 模式

Executor DIRECT_EXECUTOR = Runnable::run;

// 将被触发的事件分发给相应的 EventListener 对象
...
// 工具方法，用于获取 EventDispatcher 接口的默认实现
...
![Drawing 4.png](assets/第45讲-05.png)
...
// 获取通知 EventListener 的线程池，默认为串行模式，也就是 direct 实现
...
executor.execute(() -> {
    ...
    sortedListeners(entry -> entry.getKey().isAssignableFrom(event.getClass()))
            ...
            .forEach(listener -> {
                ...
                if (listener instanceof ConditionalEventListener) { // 针对 ConditionalEventListener 的特殊处理
